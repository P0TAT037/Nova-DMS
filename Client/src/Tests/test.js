...
function Test(info){
  ...
  var hlep = JSON.parse(ocr)
  console.log(hlep)


    //{hlep.map((line, index) =>
    //<div key={index} className="ocr-div" title={line.content} style={{position:"absolute", top: `${-hlep[0].startlocy + 655}px`, left: `${hlep[0].startlocx-200}px` ,width: `${hlep[0].width + 90}px`, height: `${hlep[0].height - hlep[0].startlocy}px`}}></div>    
    //)}
    


    //<div className="ocr-div" title={hlep[0].content} style={{position:"absolute", top: `${-hlep[0].startlocy + 655}px`, width:`${hlep[0].width + 90}px` ,height:`${hlep[0].height - hlep[0].startlocy}px` ,left: `${hlep[0].startlocx-200}px`} }></div>
    return(
