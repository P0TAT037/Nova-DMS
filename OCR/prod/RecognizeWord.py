...
act_model.load_weights('weights/best_model1.hdf5')



def recognize_words(line_indicator,word_array,n_lines, coordinates):
    ...
    for wordidxs in out:
        ...
        word=''.join(word)
        spellCorrection=spell.correction(word)
        word = word if spellCorrection == None else spellCorrection  
    ...
    for listidx in range(n_lines):
        ...
        # txt += line + '\n'
        txt.append({
            'startlocx': coordinates[listidx][0],
            'startlocy': coordinates[listidx][1],
            'width': coordinates[listidx][2],
            'height': coordinates[listidx][3],
            'content': line})
    return txt
    # file.close()
